...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await add_user(user_id, update.effective_user.username, update.effective_user.first_name, update.effective_user.last_name)

    await update.message.reply_text(
        "👋 Привет! Это бот системы доверия для сделок. Выбери нужное действие снизу.",
...
async def build_profile_text(username: str) -> str:
    ...
    return (
        ...
        f"🏷 <b>Роли:</b>\n{role_text}\n\n"
        f"🏆 <b>Уровень доверия:</b> {level} ({score})\n"
        f"📅 <b>В системе:</b> {profile['age_text']}\n"
        f"⭐ <b>Репутация:</b> {profile['reputation']}\n"
        f"🤝 <b>Сделок завершено:</b> {profile['completed_deals']}\n"
        f"⚖️ <b>Споров:</b> {profile['disputes']}\n"
        f"💬 <b>Отзывы:</b> {profile['reviews_total']} (+{profile['positive_reviews']} / -{profile['negative_reviews']})\n\n"
        f"📜 <b>История ролей:</b>\n{hist_text}"
    )
...
async def check_finish(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(await build_profile_text(username), parse_mode='HTML')
    return await return_to_main_menu(update, context)
